	...
	<div class="container">
		...
		<form action="reservation" method="get" class="search-sort-form">
			...
			<div>
				<label for="sortBy">ソート基準:</label> <select id="sortBy" name="sortBy">
					<option value=""
						<c:if test="${sortBy == null || sortBy == ''}">selected</c:if>>選択してください</option>
					<option value="name"
		...
		<div class="pagination">
			...
			<c:if test="${currentPage lt noOfPages}">
				<a
					href="reservation?action=list&page=${currentPage + 1}&search=${searchTerm}&sortBy=${sortBy}&sortOrder=${sortOrder}">次へ</a>
			</c:if>
		</div>
